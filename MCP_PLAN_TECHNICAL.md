...
Phases 0–3 are implemented on this branch. Phase 4 (write tools) is not: it needs product decisions first (see the
product plan §9). Everything below the status notes is the original plan; where the build departed from it, the
departure is listed here.

| Area | Where it lives |
|---|---|
| Principal-based access | `apps/web/lib/auth/principal.ts`; `access.ts` wraps it for sessions |
| Personal access tokens | `lib/db/schema/access.ts` (migration 0008), `app/(app)/account/token-actions.ts`, `components/account/access-tokens-card.tsx` |
| MCP endpoint and framework | `app/api/mcp/route.ts`, `lib/mcp/{http,server,auth,context,registry,params,resolve,errors,config,rate-limit,instance}.ts`, `lib/mcp/render/*` |
| Tools, prompts, resources | `lib/mcp/tools/*` (16 tools), `lib/mcp/prompts.ts`, `lib/mcp/resources.ts`, `lib/mcp/guide.ts` |
| Analysis | `lib/mcp/analysis/*` (pure, table-tested); queries in `lib/db/queries/mcp.ts` and `mcp-analysis.ts` |
| OAuth | `lib/oauth/*`, `lib/db/schema/oauth.ts` (migration 0009), `app/api/oauth/*`, `app/.well-known/*`, `app/connect/mcp/*` |
| UI | Account → Access tokens / Connected apps / AI assistants, Administration → MCP, "Debug with AI" menu, project base branch setting |
| Stdio bridge | `packages/mcp` (`@miguelfranken/mcp`, released in lockstep with the reporter) |
| Reference docs | `docs/mcp-tools.md`, generated by `nub run mcp:docs` in apps/web and guarded by `lib/mcp/docs.test.ts` |

**Departures from the plan:**

- **Queries.** The tools use two new modules, `lib/db/queries/mcp.ts` and `mcp-analysis.ts`. The plan extended
  `runs.ts` and `explorer.ts` instead. The tools filter and page differently from the UI (multi-value filters,
  offsets, windows with an end), and sharing the UI queries would couple two surfaces. Where a UI query already
  answers exactly (`getRunByNumber`, `getResultDetail`, `testHistory`, `getTestOverview`, the dashboard queries),
  the tools call it. `EXPLORER_SORTS` was not extended.
- **Rate limit.** Tool-level `RATE_LIMITED` only; the extra hard HTTP 429 at 5× the limit was not built.
- **OAuth authorization server.** Built in-house (`lib/oauth`), not with the Better Auth plugin.
  - The plugin needs Better Auth 1.7.6 (we pin 1.7.5) and brings its own schema.
  - It is unclear whether it can attach per-grant team/project restrictions and serve CIMD clients.
  - The in-house server is about 700 lines with full integration coverage.
- **Discovery documents.** Served by our own route handlers. The SDK's `oauthMetadataResponse` refuses a non-https
  issuer, which would break `http://localhost` development and self-hosted instances on private networks.
- **Base branch fallback.** `main` or `master` when they have runs, then the busiest branch. The plan only said
  "the busiest branch", which is arbitrary on a tie.
- **Token UI.** A card on `/account` instead of a separate `/account/tokens` page. Components that call Server
  Actions live in `apps/web/components`, following the existing convention (`TokensCard`); pure pieces (setup
  snippets, the hand-off menu, prompts) live in `packages/ui` with stories.

**Resolved by the build:**

- zod v4 schemas work natively in `registerTool`.
- Output validation is skipped for `isError` results.
- The legacy generation's SSE framing works with the SDK client, and with Claude Code-style clients in the
  end-to-end check.
- `originValidationResponse` lets requests without an `Origin` header through.
- Steps are stored flat with a `depth`: the failing step is the deepest one with an `error`.
- `tests.file` is relative to the Playwright config's test directory, and `npx playwright test <file>:<line>`
  matches it from the config's directory.

**Verified end to end** against a local instance with real Playwright runs:

- Token creation in the UI.
- Every tool over real HTTP on both protocol generations, including inline screenshots and signed trace links.
- The stdio bridge, with project detection from the git remote.
- The full OAuth flow through the consent page.
- The runtime switch.
